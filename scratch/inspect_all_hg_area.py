...
print("--- ALL OBJECTS IN HOOFDGEBOUW EXTENDED ZONE ---")
objs = []
for o in bpy.data.objects:
    ...
    # Check if overlapping [-40..35] x [-55..10]
    if max_x >= -40 and min_x <= 35 and max_y >= -55 and min_y <= 5:
        # filter out pure grass/road that covers entire terrain
...
objs.sort(key=lambda x: x[3]) # sort by min_y
